...
-- 1. AGGIUNGI COLONNA booking_request_id A BOOKINGS (opzionale, per tracciare richiesta originale)
DO $$
BEGIN
  IF NOT EXISTS (
...
-- 2. AGGIUNGI COLONNA booking_request_id A MESSAGES (per tracciare richieste di prenotazione)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
